...
For headless server deployment (DigitalOcean, AWS, GCP), use a separate configuration repository for your `.hermes` directory and mount it into the container.

**1. Dockerfile (Server Mode):**
```dockerfile
...
services:
  hermes:
    build: .
    volumes: [".:/root/.hermes"]
    environment: [API_SERVER_ENABLED=true, API_SERVER_PORT=8642]
    ports: ["8642:8642"]
    restart: always
```

**3. GitHub Actions CD (Manual Strike):**
Use `appleboy/ssh-action` to pull the config repo and restart the container:
```yaml
    ...
    steps:
      - uses: appleboy/ssh-action@master
        with:
          ...
          key: ${{ secrets.SSH_KEY }}
          script: |
            cd /app/hermes-config
            git pull
            docker compose up -d --build
```

**4. Connect Frontends:**
The API is OpenAI-compatible at `http://<IP>:8642/v1`. Use any string for API key unless `API_SERVER_PASSWORD` is set.
